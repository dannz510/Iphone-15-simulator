...
const WeatherSnap: React.FC = () => {
  const [temperatureF] = React.useState<number>(Math.random() * (85 - 65) + 65);
  const temperatureC = (temperatureF - 32) * (5/9);
  return (
    <span className="weather">
      <i className="weather-type" className="fa-duotone fa-sun" />
      <span className="weather-temperature-value">{temperatureC.toFixed(2)}</span>
...
const Pin: React.FC = () => {
  ...
  return(
    <div id="app-pin-wrapper">
      ...
      <h3 id="app-pin-label">Enter PIN (2205) {getErrorText()} {getCancelText()}</h3>
    </div>
  )
}
